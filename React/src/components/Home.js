...
import AllReposShow from "./allReposShow";
...
const allRepos = { key: [], value: "All" };
repositories.map(({ key }) => allRepos["key"].push(key));
repositories.push(allRepos);
    ...
    <Box sx={{ display: "flex" }}>
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
        ...
        {loading ? (
          ...
          <RepoShow githubRepoData={githubRepoData} repository={repository} />
        ) : (
          <AllReposShow githubRepoData={githubRepoData} />
        )}
      </Box>
    </Box>
  );
